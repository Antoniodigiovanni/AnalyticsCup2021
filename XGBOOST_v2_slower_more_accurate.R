...
numerics <- cbind(payments_merged1$Company_ID,payments_merged1$Number_of_licenses, specialty, payments_merged1$Physician_ID, product, product_indicator, third_party, state, form, com_state)
matrix_train <- data.matrix(numerics)


# test set is 1/kth of total set
k <- 4
SAVE_BACs <-c()
SAVE_confMat <- list()
...
n <- 5
for(i in 1:n){
randtestIndexes <- sample(1:nrow(matrix_train), size = length(numIndexes))
...
model <- xgboost(data = dtrain, # the data   
                 nround = 90, # max number of boosting iterations
                 eta = 0.2435,
...
# TODO: Maybe create about 10 distinct ranges in total amount of payments and
# try to include
# TODO: For finetuning, take the average BAC of the different rounds and compare
# TODO: Decide between taking longer vs a bit less accurate

# Done: Company_State makes the model slightly more accurate
